...
<?php if (!empty($shopkeeper)): ?>
    <?php foreach ($shopkeeper as $row): ?>
        <a href="<?= base_url('product/' . esc($row['slugus'])); ?>">
            <div class="product-card" style="width: 250px; border: 1px solid #ddd; border-radius: 10px; overflow: hidden; text-align: center; font-family: Arial, sans-serif;">
...
foreach ($images as $image) {
    $imagePath = !empty($image) ? 'assets/uploads/' . $image : 'assets/uploads/placeholder.jpg';
    echo '<img src="' . base_url($imagePath) . '" alt="Product Image" class="rounded shadow-sm" style="margin: 5px; width: 100%; height: auto;">';
}
?>
                ...
                <div class="product-details" style="margin-top: 20px;">
                    <p><strong>ID:</strong> <?= esc($row['id']); ?></p>
                    <p><strong>Slug:</strong> <?= esc($row['slugus']); ?></p>
                    <p><strong>Product Image:</strong> <?= esc($row['Productimage']); ?></p>
                    <p><strong>Product URL:</strong> <?= esc($row['producturl']); ?></p>
                    <p><strong>Product Price:</strong> ₹<?= number_format($row['productprice'], 2); ?></p>
                    <p><strong>Technique:</strong> <?= esc($row['techinque']); ?></p>
                    <p><strong>Colors:</strong> <?= esc($row['colors']); ?></p>
                    <p><strong>Washing Instruction:</strong> <?= esc($row['washinginstruction']); ?></p>
                    <p><strong>Fabric Details:</strong> <?= esc($row['fabricdetails']); ?></p>
                    <p><strong>Product Details:</strong> <?= esc($row['productdetails']); ?></p>
                    <p><strong>Delivery Timeline:</strong> <?= esc($row['deliverytimeline']); ?> days</p>
                    <p><strong>International Delivery:</strong> <?= esc($row['internationaldelivery']); ?> days</p>
                    <p><strong>Created At:</strong> <?= esc($row['created_at']); ?></p>
                    <p><strong>Updated At:</strong> <?= esc($row['updated_at']); ?></p>
                </div>
            </div>
        </a>
    <?php endforeach; ?>
